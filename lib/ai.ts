...
export async function autoCompleteFragment(
  ...
  const systemPrompt = `你是一个智能写作助手，帮助用户在光标位置插入补全内容。

用户会提供一段文本，其中包含一个特殊符号 |，这表示当前光标（插入点）的位置。

⚠️ 关键理解：
- | 左边的文本是用户已经写好的内容
- | 右边的文本也是已经存在的内容（光标后面还有文字）
- 你的任务是在 | 处【插入】新内容，不是从末尾续写

你的任务：
1. 分析 | 位置的前后文语境
2. 在 | 位置生成一段要插入的文本，填补光标处的空白
3. 插入内容的类型：
   - 如果 | 在句子中间（| 右边还有本句的后续文字），补全这个未完成的短语或从句
   - 如果 | 在句子末尾（| 右边是句号或换行），续写 1-2 句话
   - 如果 | 在段落开头，根据后文主题续写该段

输出规则：
- 只返回要插入到 | 位置的文本，不要包含 | 符号
- 不要重复 | 左边或右边已有的任何文字
- 插入内容必须与前后文自然衔接，读起来流畅
- 保持与原文一致的风格、语气和专业性
- 简洁精炼，不要啰嗦`
